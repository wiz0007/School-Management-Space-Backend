...
public class StaffService {
	...
	public StaffService(StaffRepository staffRepository, WorkspaceAccessService workspaceAccessService) {
	...
	public StaffResponse update(UserAccount owner, UUID id, StaffRequest request) {
		SchoolProfile schoolProfile = schoolProfileFor(owner);
		StaffMember staff = workspaceAccessService.requireStaff(schoolProfile, id, "Staff record not found.");
	...
	private SchoolProfile schoolProfileFor(UserAccount owner) {
		return workspaceAccessService.requireSchoolProfile(owner, "Create the school profile before adding staff.");
	}
